	...
	if len(proxy.serversInfo.registeredServers) > 0 {
		go func() {
			for {
				...
				runtime.GC()
				if !proxy.readyFired && liveServers > 0 && err == nil {
					proxy.readyFired = true
					proxy.ReadyCallback <- true
					close(proxy.ReadyCallback)
				}
	...
	for {
		...
		case a := <-c:
			...
			proxy.workerPool.Execute(func() {
				...
				attempt := 0
				for {
					_, err := proxy.processIncomingQuery("udp", proxy.mainProto, packet, &clientAddr, clientPc, start, attempt)
	...
	for {
		...
		case a := <-c:
			...
			proxy.workerPool.Execute(func() {
				...
				attempt := 0
				for {
					_, err := proxy.processIncomingQuery("tcp", "tcp", packet, &clientAddr, clientPc, start, attempt)
...
func (proxy *Proxy) processIncomingQuery(clientProto string, serverProto string, query []byte, clientAddr *net.Addr, clientPc net.Conn, start time.Time, attempt int) (response []byte, errOut error) {
	...
	if proxy.retryCount > 0 && attempt >= proxy.retryCount { //TODO: plugin?
		...
		if err != nil {
			return //TODO: wtf? O_o should't happen
		}
		response, err = refMsg.PackBuffer(response)
		if err != nil {
			return //TODO: wtf? O_o should't happen
		}
	}
	...
	if len(response) == 0 && serverInfo != nil {
		...
		if serverInfo.Proto == stamps.StampProtoTypeDNSCrypt {
			...
			serverInfo.noticeBegin(proxy)
			serverResponse, tls, _, err := proxy.xTransport.DoHQuery(serverInfo.useGet, serverInfo.URL, query, proxy.timeout, serverInfo.Name)
